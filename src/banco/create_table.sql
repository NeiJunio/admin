...
CREATE TABLE IF NOT EXISTS `agenda_servicos` (
	...
	`agend_situacao` tinyint NOT NULL,
	PRIMARY KEY (`agend_id`, `serv_id`)
);
...
ALTER TABLE `agenda_servicos` ADD CONSTRAINT `agenda_servicos_fk0` FOREIGN KEY (`agend_id`) REFERENCES `agendamentos`(`agend_id`);

ALTER TABLE `agenda_servicos` ADD CONSTRAINT `agenda_servicos_fk1` FOREIGN KEY (`serv_id`) REFERENCES `servicos`(`serv_id`);
